...
Raw Text: ${homepageText.slice(0, 15000)}`;
    ...
    for (const item of updatesToProcess) {
      ...
      const detailPrompt = `You are a detailed data extractor. Extract specific information from this job detail page text and return ONLY a strict JSON object. 
Ensure the JSON keys exactly match these: 
- lastDate: (string, e.g., '15-08-2026')
- importantDates: (string, summarized important dates)
- applicationFee: (string, summarized fee details)
- eligibility: (string, concise eligibility/age limit)
- vacancyDetails: (string, concise summary of vacancies/posts)
...
Raw Text: ${detailText.slice(0, 15000)}`;
      ...
      if (deepDetails) {
        await fetchFromMongo('insertOne', {
          document: {
            ...
            importantDates: deepDetails.importantDates || 'Not Specified',
            applicationFee: deepDetails.applicationFee || 'Not Specified',
            eligibility: deepDetails.eligibility || 'Not Specified',
            vacancyDetails: deepDetails.vacancyDetails || 'Not Specified',
